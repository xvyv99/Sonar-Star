...
from feature_extract.basic_functions import *
...
pip install mpl_toolkits==2.0.0
...
class SpectralSubtraction:
    ...
    def saveWave(self, output_path):
        sf.write(output_path, self.output.astype(np.float32), self.fs)
...
"""
预加重 系数0.97、分帧、加窗 加汉明窗、FFT快速傅里叶变换
"""
...
class Spectrogram:
    ...
    def get_mel_spectrogram(self, n_mels=26):
        ...
        mel_spec = librosa.feature.melspectrogram(y=self.wave_data, sr=self.sr, n_fft=self.fft_num,
    ...
    # , savepath
    def plot(self, savepath, fig=None, show=True, **kwargs):
        """
        绘制声谱图
        :param fig: 指定绘制何种声谱图，mag/pow/log_pow/mel,默认都绘制
        :param show: 默认最后调用plt.cla()，显示图形
        :return: None
        """
        if fig == "mag":
            ...
            # plt.gca()
            # plt.savefig(savepath + "mag_spec.png")

            # Power Spectrogram - 功率图
            # 不保存图片
            # plt.subplot(2, 2, 2)
            # pow_spec = self.get_power_spectrogram()
            # librosa.display.specshow(pow_spec, sr=self.sr, hop_length=self.hop_length, x_axis="s", y_axis="linear")
            ...
            # 保存图片
            # pow_spec = self.get_power_spectrogram()
            # librosa.display.specshow(pow_spec)
            # plt.gca()
            # plt.savefig(savepath + "pow_spec.png")

            # Log-Power Spectrogram - 能量对数谱图
            # 不保存图片
            # plt.subplot(2, 2, 3)
            # log_pow_spec = self.get_log_power_spectrogram()
            ...
            # 保存图片
            # log_pow_spec = self.get_log_power_spectrogram()
            # librosa.display.specshow(log_pow_spec)
            # plt.gca()
            # plt.savefig(savepath + "log_pow_spec.png")

            # Mel Spectrogram - 梅尔谱图
            # 不保存图片
            # plt.subplot(2, 2, 4)
            # mel_spec = self.get_mel_spectrogram(**kwargs)
            ...
            # librosa.display.specshow(mel_spec)
            # plt.gca()
            # plt.savefig(savepath + "mel_spec.png")
        plt.xticks([])  # 去掉x轴
        plt.yticks([])  # 去掉y轴
        plt.axis('off')  # 去掉坐标轴
...
class RhythmFeatures:
    ...
    def plot(self, savepath, energy="ste", show=True):
        """
...
class SpectrumFeatures:
    ...
    # , savepath1
    def plot(self, savepath1, show=True, **kwargs):
        """
        *
        绘制MFCC特征图
        :param show: 默认最后调用plt.cla()，显示图形
...
class QualityFeatures:
    ...
    def formant(self, ts_e=0.01, ts_f_d=200, ts_b_u=2000):
        ...
        for i in range(n_frame):
            ...
            f_i_win = f_i * win  # 加窗
            a = librosa.lpc(f_i_win, order=8)  # 获取LPC线性预测误差系数，即滤波器分母多项式，阶数为 预期共振峰数3 *2+2，即想要得到F1-3
    ...
    def plot(self, savepath1, savepath2, show=True):
        """
...
def my_acoustic_features(input_file, sr=None, frame_len=512, n_fft=None, win_step=2 / 3, window="hamming",
    ...
    fmt_f1_d2, fmt_b1_d2 = librosa.feature.delta(fmt_f1, width=3, order=2), librosa.feature.delta(fmt_b1, width=3,
                                                                                                  order=2)
...
def get_base64(wav_path):
    ...
    # filename_ending = filenames
    #
    # filename = copy.copy(filenames)
    # 保存地址

    # savepath_ending = filenames
    # print(filenames)
    # print(len(filenames))

    # print(filenames[i])
    # # 音频源文件地址
    # filename_ending[i] = wav_path + filenames[i]
    # # 读取文件的地址
    # wave_file = copy.copy(filename_ending[i])
    # # print(wave_file)
    # # filenames = os.listdir(filepath)
    # filenames[i] = filename[i]
    # print("filenames[i]:"+filenames[i])
    # 更换文件名
    filename = wav_path.replace(".wav", "")
    # jpg图片保存地址
    # print(filename[i])
    # /usr/storage/1.wav
    # /usr/storage/temp
    # 设置wav_path的根目录
    root_path = wav_path.replace(os.path.abspath(wav_path).split("/")[-1], "")
    print('root_path:'+root_path)
    # print(root_path)
    # 如果不存在进行创建
    if os.path.exists(root_path + r"temp"):
        pass
    else:
        os.mkdir(root_path + r"temp")

    savepath_mfcc = root_path + "temp/mfcc.png"
    print('在程序内部的savepath_mfcc:'+savepath_mfcc)
    # 原始波形图
    savepath_orgin_wave = root_path + "temp/orginWave.png"
    # 降噪波形图
    savepath_reduce_wave = root_path + "temp/reduceWave.png"
    # 端点检测图
    savepath_endpoint_detection = root_path + "temp/endpointDetection.png"
    # 短时平均能量
    savepath_short_time_energy = root_path + "temp/short_time_energy.png"
    # 对数功率谱图
    savepath_spec = root_path + "temp/spec.png"
    # 波形图
    savepath_wave_curve = root_path + "temp/wave_curve.png"
    # 声谱图
    savepath_power_spec = root_path + "temp/power_spec.png"
    # print(savepath_ending[i])
    # filenames = os.listdir(filepath)
    # jpg文件名
    # filenames[i] = filenames[i].replace('.','') + '.jpg'
    # savepath_ending[i] = savepath_ending[i].replace(".wav","")
    # savepath_ending[i] = savepath_ending[i] + filenames[i]
    # print("jpg文件名：" + filenames[i])
    # filenames = os.listdir(filepath)
    # print("文件保存的地址：" + savepath_ending[i])
    # print(type(savepath_ending[i]))
    # Berouti谱减法降噪算法
    vad_filenames = copy.copy(filepath)
    ...
    displaySpeech(x, 8000, savepath_orgin_wave)
    # displaySpeech(x1, 8000)
    displaySpeech(x2, 8000, savepath_reduce_wave)
    # print(wav_path.split(".")[0])
    vad_filenames = wav_path.split(".")[0] + "_spectral.wav"
    ss.saveWave(vad_filenames)
    # print("wav_file:"+wave_file)

    # wave_file_vad = wav_path.split(".")[0] + "_vad.wav"
    wave_file_vad = vad_filenames.split(".")[0] + "_vad.wav"
    ...
    vad.plot(savepath_endpoint_detection)
    # 利用openSmile工具进行特征提取
    # opensmile_f = OpenSmileFeatureSet(vad_filenames[i])
    ...
    rhythm_f.plot(savepath_short_time_energy)
    img = cv2.imdecode(np.fromfile(savepath_short_time_energy, dtype=np.uint8), cv2.IMREAD_COLOR)
    img = img[0:330, 0:800]
    # os.remove(savepath_short_time_energy)
    cv2.imencode(".png", img)[1].tofile(savepath_short_time_energy)
    #  基于谱的相关特征,画出MFCC特征
    # 不保存图片
    # spectrum_f = SpectrumFeatures(vad_filenames[i])
    # spectrum_f = SpectrumFeatures(wave_file_vad)
    # spectrum_f.plot()
    # 保存图片
    spectrum_f = SpectrumFeatures(vad_filenames)
    # spectrum_f = SpectrumFeatures(wave_file_vad)
    spectrum_f.plot(savepath_mfcc)

    # with open(savepath_mfcc, 'rb') as f:
    #     base64_data = base64.b64encode(f.read())
    #     base64_str = base64_data.decode('UTF8')
    # os.remove(savepath_mfcc)

    # os.remove(root_path + r"\temp")

    # 音质特征
    # 不保存图片
    quality_f = QualityFeatures(wave_file_vad)
    quality_f.plot(savepath_wave_curve, savepath_power_spec)
    img = cv2.imdecode(np.fromfile(savepath_wave_curve, dtype=np.uint8), cv2.IMREAD_COLOR)
    img = img[200:425, 75:750]
    # os.remove(savepath_wave_curve)
    cv2.imencode(".png", img)[1].tofile(savepath_wave_curve)
    img = cv2.imdecode(np.fromfile(savepath_power_spec, dtype=np.uint8), cv2.IMREAD_COLOR)
    img = img[25:435, 50:800]
    # os.remove(savepath_power_spec)
    cv2.imencode(".png", img)[1].tofile(savepath_power_spec)
    # 保存图片
    # quality_f = QualityFeatures(wave_file_vad)
    # quality_f.plot(savepath_ending[i])

    # 声谱图特征
    # 不保存图片
    spectrogram_f = Spectrogram(wave_file_vad)
    spectrogram_f.plot(savepath_spec)
    img = cv2.imdecode(np.fromfile(savepath_spec, dtype=np.uint8), cv2.IMREAD_COLOR)
    img = img[75:725, 175:1470]
    # os.remove(savepath_spec)
    cv2.imencode(".png", img)[1].tofile(savepath_spec)
    # 保存图片
    # spectrogram_f = Spectrogram(wave_file_vad)
    # spectrogram_f.plot(savepath_ending[i])
    os.remove(vad_filenames)
    os.remove(wave_file_vad)
    return savepath_mfcc,savepath_orgin_wave,savepath_reduce_wave,savepath_endpoint_detection,savepath_short_time_energy,savepath_spec,savepath_wave_curve,savepath_power_spec


if __name__ == "__main__":
    print('程序返回的地址：'+str(get_base64(r'G:\Maker\自闭症语音识别\数据集\ASDChildren\ASDChild_1.wav')))
