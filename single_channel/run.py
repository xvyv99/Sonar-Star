...
def main():
    parser = argparse.ArgumentParser(description='一键运行单通道模型的完整流程')
    ...
    if not args.skip_data_prep:
        # 遍历音频目录中的所有音频文件
        audio_files = [f for f in os.listdir(args.audio_dir) if f.endswith(('.wav', '.mp3', '.flac'))]
        for audio_file in audio_files:
            audio_path = os.path.join(args.audio_dir, audio_file)
            run_command(
                f"python -m single_channel.extract_mfcc --audio {audio_path} --output {args.output_dir}",
                f"提取MFCC特征: {audio_file}"
            )
    else:
        print("\n跳过数据准备步骤...")
    ...
    if not args.skip_training:
        run_command(
            f"python -m single_channel.train --data_dir {args.output_dir} --epochs {args.epochs} "
            f"--batch_size {args.batch_size} --lr {args.lr} --save_dir {model_dir}",
            "步骤2: 训练单通道SE-ResNet50模型"
        )
    else:
        print("\n跳过模型训练步骤...")
    ...
    if args.test_audio:
        ...
        if os.path.exists(model_path):
            run_command(
                f"python -m prediction.predict_single_channel --audio {args.test_audio} --model {model_path} --output {args.output_dir}",
                "步骤3: 使用单通道模型进行自闭症风险预测"
            )
        else:
